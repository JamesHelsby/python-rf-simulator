...
def plot_data():
    ...
    grid_x, grid_y = np.meshgrid(
        np.linspace(num_jamming.min(), num_jamming.max(), 100),  # x-axis: num_jamming
        np.linspace(power.min(), power.max(), 100)  # y-axis: power
    )

    # Interpolate B (z-axis) values onto the grid
    grid_z = griddata((num_jamming, power), B, (grid_x, grid_y), method='linear')
    grid_class = griddata((num_jamming, power), classification_numeric, (grid_x, grid_y), method='nearest')

    colormap = ListedColormap(['green', 'red', 'purple', 'orange'])

    fig = plt.figure()
    ax = fig.add_subplot(111, projection='3d')
    surf = ax.plot_surface(grid_x, grid_y, grid_z, facecolors=colormap(grid_class.astype(int)), alpha=0.8)

    colors = classification.map({'green': 'green', 'red': 'red', 'purple': 'purple', 'orange': 'orange'})
    ax.scatter(num_jamming, power, B, s=10, c=colors, edgecolor='k', linewidths=0.1)

    ax.view_init(elev=ELEVATION, azim=AZIMUTH)

    x_label = r'$\mathcal{M}$ (Proportion malicious)'
    y_label = 'Power (dBm)'
    z_label = r'$B$ (Proportion disconnected)'
    ...
    plt.savefig('plot.svg', bbox_inches='tight', dpi=1000)
